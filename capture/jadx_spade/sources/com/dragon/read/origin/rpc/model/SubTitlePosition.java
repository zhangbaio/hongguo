package com.dragon.read.origin.rpc.model;

import com.bytedance.covode.number.Covode;

/* loaded from: D:\code\hongguo\capture\classes6.dex */
public enum SubTitlePosition {
    VideoSeriesPostMainDramaCard(1),
    VideoSeriesPostAnchorList(2),
    VideoSeriesPostMainDramaCardVertical(3),
    VideoSeriesPostMainDramaCardNew(4);

    private final int value;

    public int getValue() {
        return this.value;
    }

    static {
        Covode.recordClassIndex(613463);
    }

    public static SubTitlePosition findByValue(int i) {
        if (i != 1) {
            if (i != 2) {
                if (i != 3) {
                    if (i != 4) {
                        return null;
                    }
                    return VideoSeriesPostMainDramaCardNew;
                }
                return VideoSeriesPostMainDramaCardVertical;
            }
            return VideoSeriesPostAnchorList;
        }
        return VideoSeriesPostMainDramaCard;
    }

    SubTitlePosition(int i) {
        this.value = i;
    }
}
