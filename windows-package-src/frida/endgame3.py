# -*- coding: utf-8 -*-
"""原始16字节密钥暴力扫描: 抓明文+序列匹配恢复keystream + 同步dump内存
+ 对每个16字节对齐窗口做 counter-diff 验证(找原始key, 不依赖轮密钥结构)。"""
...
adb1("su -c 'rm -f /data/local/tmp/e3.h265; touch /data/local/tmp/e3.h265; chmod 777 /data/local/tmp/e3.h265'")

JS=r"""
'use strict';
var fv=null,vBytes=0;
Java.perform(function(){
  try{var FOS=Java.use('java.io.FileOutputStream'); fv=FOS.$new('/data/local/tmp/e3.h265',false);}catch(e){}
  var MC=Java.use('android.media.MediaCodec');
  MC.flush.implementation=function(){
    try{var FOS=Java.use('java.io.FileOutputStream'); if(fv){fv.flush();fv.close();} fv=FOS.$new('/data/local/tmp/e3.h265',false);}catch(e){}
  ...
  MC.queueInputBuffer.implementation=function(idx,off,size,pts,flags){
    if(size>4){try{var n=this.getName().toLowerCase();
      if(/hevc|bytevc|h265/.test(n)){var bb=this.getInputBuffer(idx);bb.position(off);
        var a=Java.array('byte',new Array(size).fill(0));bb.get(a);if(fv){fv.write(a,0,size);vBytes+=size;}}}catch(e){}}
...
open("capture/e3_dl.txt","w",newline="\n").write(dl)
subprocess.run([ADB,"-s",DEV,"push","capture/e3_dl.txt","/data/local/tmp/e3_dl.txt"])
...
open("capture/e3.sh","w",newline="\n").write(sh)
subprocess.run([ADB,"-s",DEV,"push","capture/e3.sh","/data/local/tmp/e3.sh"])
subprocess.run([ADB,"-s",DEV,"shell","su -c 'sh /data/local/tmp/e3.sh'"],timeout=120)
subprocess.run([ADB,"-s",DEV,"pull","/data/local/tmp/e3.gz","capture/e3.gz"])
with gzip.open("capture/e3.gz","rb") as f,open("capture/e3.bin","wb") as g: shutil.copyfileobj(f,g,1<<20)
dsz=os.path.getsize("capture/e3.bin"); print(f"dump {dsz//1048576}MB")

# 明文 + 序列匹配
plain=subprocess.run([ADB,"-s",DEV,"exec-out","su -c 'cat /data/local/tmp/e3.h265'"],capture_output=True).stdout
open("capture/e3_plain.h265","wb").write(plain)
pnals=[];i=0
while i+5<len(plain):
...
print(f"plain {len(plain)}B {len(pnals)} nals")
def ib(d,s,e):
    o=s
    while o+8<=e:
...
WIN=8; blkA=blkB=None; blkA_idx=None
for url in list(st["urls"]):
    ...
    try:
        ...
        pi,ci=align;po,psz,pt=pnals[pi];co=offs[ci];csz=sizes[ci]
        ...
        print(f"[seqmatch] plain#{pi}<->sample#{ci} blkA={blkA.hex()} blkA_idx={blkA_idx}")
        json.dump({"blkA":blkA.hex(),"blkB":blkB.hex(),"blkA_idx":blkA_idx},open("capture/e3_ks.json","w"))
    except: pass
if not blkA: print("no seqmatch — 重试"); raise SystemExit

# === 原始16字节密钥暴力扫描 (counter-diff, 16字节对齐) ===
print(f"brute-forcing raw 16B key over {dsz//1048576}MB (16-aligned, counter-diff)...")
t0=time.time(); found=None; tested=0
with open("capture/e3.bin","rb") as f:
    CH=64<<20; carry=b""; gpos=0
    while True:
        chunk=f.read(CH)
        if not chunk: break
        buf=carry+chunk
        ...
        for o in range(start,lim+1,16):
            K=buf[o:o+16]
            try:
                d=AES.new(K,AES.MODE_ECB)
                cA=int.from_bytes(d.decrypt(blkA),"big")
                cB=int.from_bytes(d.decrypt(blkB),"big")
            except: continue
            tested+=1
            if (cB-cA)%(1<<128)==1:
                iv=(cA-blkA_idx)%(1<<128)
                found=(K,cA,iv)
                print(f"\n*** RAW CONTENT KEY FOUND! key={K.hex()}")
                print(f"    IV={iv:032x}  counter@blk{blkA_idx}={cA:032x}")
                break
        if found: break
        carry=buf[-16:] if len(buf)>=16 else buf
        gpos+=len(chunk)
        if tested and tested%2000000<16: print(f"  tested {tested//1000000}M, {time.time()-t0:.0f}s")
print(f"brute done: tested={tested} in {time.time()-t0:.0f}s")
if found:
    json.dump({"key":found[0].hex(),"iv":f"{found[2]:032x}","blkA_idx":blkA_idx,"counterA":found[1],"source":"raw_brute"},open("capture/CONTENT_KEY.json","w"),indent=2)
    print("saved capture/CONTENT_KEY.json !!! 原始密钥已提取")
else:
    print("16-aligned brute found nothing. 可能key非16对齐(改4对齐重扫)或IV/CTR模型不同")
